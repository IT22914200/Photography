  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const userPosts = await photographyPostApi.getPostsByUser(id);
        setPosts(userPosts);
      ...
      <div className="flex flex-col sm:flex-row items-center sm:items-start mb-8">
        ...
        <div className="flex-1 text-center sm:text-left">
          ...
          <div className="mb-4">
            ...
            {user.badge && (
              <div className="inline-block bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded-full mt-1">
                {user.badge}
              </div>
            )}
            <p className="text-gray-600 mt-2">
              {user.bio || "Photography enthusiast and photo lover."}
            </p>
          </div>
        </div>
      </div>
